...
class MMMUDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
            ...
            for idx, pil_image in enumerate(pil_images):
                if pil_image is not None:
                    pil_image = dynamic_preprocess(pil_image, image_size=self.input_size,
                                                   ...
                                                   min_num=4, max_num=self.max_num)
                    images += pil_image
        ...
        pixel_values2 = [self.transform(image) for image in images2]
        pixel_values2 = torch.stack(pixel_values2)
        pixel_values = torch.cat([pixel_values[:-1], pixel_values2, pixel_values[-1:], pixel_values[-1:]], 0)
